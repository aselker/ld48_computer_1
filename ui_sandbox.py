...
for y, names in enumerate(box_names):
    for x, name in enumerate(names):
        is_highlighted = x == 1 and y == 1
        box_color = term.black_on_white if is_highlighted else term.white_on_black
        print(term.move_xy(10 + 30*x, 5 + 13*y))
        print(box_color(" "*10))
        print(term.move_xy(10 + 30*x, 6 + 13*y))
        print(box_color(" ") + term.white_on_black(name))
        print(term.move_xy(18 + 30*x, 6 + 13*y))
        print(box_color(" "))
        print(term.move_xy(10 + 30*x, 7 + 13*y))
        print(box_color(" "*10))
